...
```{r}
df <- read.csv(file = "C:\\Users\\PheXeRiaN\\Desktop\\Data Science\\Thesis\\CleanedOVData.csv")
```
...
plot_bar(df$Race)  ## Looking at race column in dataframe.  Mostly causacian females.
```
...
### Looking at second set of data with COL12A1Duplication exp(coef) = 0.6977 we can say that having a duplicated CNV count is 0.70 times better chances of survival.  This is the hazard ratio.  This equates to a 30% chance less likely to die with a duplicated CNV of some kind.  The exp(-coef) = 1.433 means that someone without a duplication has a 43% increased chance of dying.
...
plot(CoxsurvCOL12A1, col = c("black", "red", "blue"))
```
...
### The beta lines seem to stay within the 95% confidence intervals at all times on this plot of hazard ratio over time.  Our assumptions are met with this model.  We also can see each strafied group in the cox hazard over time model for this gene.
```
